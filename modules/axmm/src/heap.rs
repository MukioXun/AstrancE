...
pub struct HeapSpace { max_heap_size: usize, heap_base: AtomicUsize, heap_top: AtomicUsize, }
...
impl HeapSpace {
    ...
    pub(crate) fn 
    set_heap_top(&self, top: VirtAddr) -> VirtAddr {
        assert!(
...
impl AddrSpace {
    ...
    pub fn set_heap_top(&mut self, top: VirtAddr) -> VirtAddr {
        ...
        if(top != self.heap().top()){
            self.areas.adjust_area(self.heap().base(), self.heap().base(), top.align_up_4k(), &mut self.pt).unwrap();
