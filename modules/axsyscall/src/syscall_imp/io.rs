...
pub fn ae_read(fd: usize,buf:&mut[u8]) -> SyscallResult {
    let ret: isize;
    //检查读取的位置的权限安全性
...
pub fn ae_write(fd: usize,buf:&[u8]) -> SyscallResult {
    let ret: isize;
    if fd == 1 || fd == 2{
...
pub fn ae_writev(fd: c_int, iov: *const ctypes::iovec, iocnt: c_int) -> SyscallResult {
